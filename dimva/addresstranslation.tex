...
While we discussed in the last section methods to gain access to a computer's
memory and their limitations, we will discuss in this section a method for
structured access to these sources.
...
%For a detailed description and discussion of these two important mechanisms,
%the reader may refer to a course on system programming, e.g\@.
...
\emph{Intel 64 and IA-32 Architectures Software Developer's Manual} \linebreak
...
for all pages we make a simple test if a page \emph{could} be a pagedirectory
...
\begin{itemize}
	...
	\item \emph{code and heap} will be starting around \texttt{0x0800~0000},
		consecutively following with a minor number of unused frames in
		between

	\item \emph{libraries} and \emph{custom mappings} will be mapped below
		\texttt{0xb800~0000}

	\item the \emph{userspace stack} will be mapped below
