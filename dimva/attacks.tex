...
\begin{enumerate}
	...
	agent. % (see figure \ref{fig:code:identity-struct}). 
...
%\end{center}\end{figure}
	...
	purpose cryptography library (\text{libcrypto}).}-structures \texttt{RSA} and
	\texttt{DSA}.
...
%\end{center}\end{figure}
	...
	\item For both RSA and DSA structures %(figure \ref{fig:code:rsa-struct} and
	%\ref{fig:code:dsa-struct}) 
...
%(\texttt{attacks/userspace/shellcodes/dmashellcode.s}) is introduced in
%appendix \ref{beachhead_pseudocode} in pseudo-code.  The program to inject the
