...
stripping also strips any information about the \emph{idtable} symbol.

Furthermore once \emph{one} \emph{identity}-struct is found, \emph{all} structs
of the same keytype (RSA or DSA) can be found by walking the list this key is
linked into.  This has only been omitted so far for reasons of \emph{me missing
...
key in a format directly usable by \texttt{ssh} it is required to link against
