...
% show a tree with the structure of the data that is obtained
\texttt{ \small
		\Tree [.{struct identity}
			[.{Key\\ *key}
				{RSA\\ *rsa} !{\qframesubtree}
				{int\\type}
				{int\\flags}
				{DSA\\ *dsa} !{\qframesubtree}
			] !{\qsetw{3cm}}
			{char\\ *comment} !{\qframesubtree}
			{int\\death}
		]
} \\ [3ex]
...
\texttt{ \small
		\Tree [.BIGNUM
			{ulong\\ *d} {int\\ top} {int\\ dmax} {int\\ neg} {int\\ flags}
		]
}
...
\begin{enumerate}
	...
	general purpose cryptography library (\text{libcrypto}).}-structures
	...
	implementation is the \texttt{BIGNUM}-struct (abbreviated ``BN'' in the
	trees above). It consists of a variable-length array of bit-vectors
	forming the value and a length-field defining the length of this array.
