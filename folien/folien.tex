...
\pgfdeclareimage[height=6.0cm]{linux-ia32-vmlayout}{linux_virtual_address_space}
\pgfdeclareimage[width=10.0cm]{beachhead}{functionality_beachhead}
...
\section{Virtual address spaces}
	...
	\begin{frame}
		We could parse kernel data-structures (if we find them\ldots). But they are different for different
		\begin{itemize}
	...
	\subsection{Virtual address spaces}
		...
		\begin{frame}{Virtual Address Spaces}
			\begin{itemize}
				...
				\item \structure{privilege separation} required
...
\section{Gathering information}
	...
	\subsection{Identifying Processes}
		...
		\begin{itemizeframe}{}
			\item (argv, envv) are somewhere in the address space
		...
		\begin{frame} \frametitle{Stealing SSH private keys}
			...
			\uncover<2->{
				...
				\begin{itemize}
					...
					\item \alert{no timer} to check for timeout
		...
		\begin{frame} \frametitle{finding SSH Private keys [2]}
			\begin{itemize}
				...
				\item[5] A lot of \texttt{BIGNUM}s (OpenSSL arbitrary precision integer implementation).
					 \textcolor{blue}{Copy them, test integrity}.
				\item[6] 0wn3d
			\end{itemize}
...
%		\item The pen is mightier than the sword.
	...
	\subsection{Executing the code}
		...
		\begin{itemizeframe}{Executing injected code}
			\item Use program-flow
			\item Typical process calls subroutines
			\item Stackframes on stack, return-address
			\item overwrite return-addresses
		\end{itemizeframe}

		\begin{itemizeframe}{Protection Mechanisms}
			\item Stackoverflow checksums:
			\begin{itemize}
